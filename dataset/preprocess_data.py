...
def scale_crop(clip, train, opt):
    ...
    if opt.modality == 'RGB':
        ...
        processed_clip = torch.Tensor(8, int(len(clip)/3), opt.sample_size, opt.sample_size)
        ...
        for i, I in enumerate(clip):
            ...
            if opt.modality == 'RGB':
                ...
                if j == 0:
                    ...
                    I = Normalize(get_mean('activitynet'), [1,1,1,])(I)
                    processed_clip[3:6, int(i/3), :, :] = I
                else:
                    I = Normalize([127.5, 127.5, 127.5], [1,1,1])(I)
                    if j == 2:
                        processed_clip[6, int((i-1)/3), :, :] = I
                    elif j == 3:
                        processed_clip[7, int((i-2)/3), :, :] = I
                j += 1
                if j == 4:
                    j = 0
